...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside className={`fixed top-0 left-0 h-full bg-zinc-900 flex flex-col text-white transition-all duration-300 ease-in-out z-30 ${isOpen ? 'w-[var(--sidebar-open-width)]' : 'w-[var(--sidebar-closed-width)]'}`}>
        <div className="h-full flex flex-col">
            {/* Header: New Chat */}
            <div className="flex-shrink-0 p-2 border-b border-zinc-800 h-[65px] flex items-center">
                 {isOpen ? (
                    <button
                        onClick={onNewChat}
                        className="w-full flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md text-white bg-zinc-800 hover:bg-zinc-700 transition-colors"
                    >
                        <PlusIcon />
                        New Chat
                    </button>
            ...
            <div className="flex-1 min-h-0 overflow-y-auto p-2">
                ...
                <div>
                    {/* FIX: Removed sticky positioning to prevent overlap on scroll */}
                    <p className={`text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-2 py-1 ${isOpen ? 'px-3' : 'text-center'}`}>
            ...
            <div className="flex-shrink-0 border-t border-zinc-800 p-2 space-y-1">
                <button
                    onClick={onOpenAboutVandvikModal}
                    title="About Vandvik"
                ...
                >
                    <InfoIcon />
                    {isOpen && <span className="truncate">About Vandvik</span>}
                </button>
                <button
